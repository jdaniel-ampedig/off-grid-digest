...
on readConfig()
    try
        ...
        repeat with ln in rows
            ...
            if lineText is "" then
                ...
                if eqPos > 0 then
                    ...
                    else if my iequals(k, "senderEmail") then
                        set senderEmailStr to v
                    end if
                end if
            end if
        end repeat
        ...
        return {enabledBool, startStr, endStr, emailStr, zoleoNumberStr, senderEmailStr}
...
on run
    ...
    set senderEmail to item 6 of cfg
    ...
    try
        ...
        if raw is "" then
            ...
            if (do shell script "[ -r " & quoted form of callDBPath & " ] && echo 0 || echo 1") is "0" then
                ...
                if (count of missedList) > 0 then
                    ...
                    my sendDigestEmail(subjectStr, contentStr)
                    (*
        ...
        -- set callDBPath to POSIX path of (path to library folder from user domain) & "Application Support/CallHistoryDB/CallHistory.storedata"
        ...
        tell application "Mail"
            --set theMessage to make new outgoing message with properties {visible:false, subject:subjectStr, content:contentStr}
            set theMessage to make new outgoing message with properties {visible:false, sender:"josh.daniel@ampedig.com", subject:subjectStr, content:contentStr}
            tell theMessage to make new to recipient at end of to recipients with properties {address:targetEmail}
            send theMessage
        end tell
        *)
...
on sendDigestEmail(subjectStr, contentStr)
    tell application "Mail"
        if senderEmail is not "" then
            set theMessage to make new outgoing message with properties {visible:false, sender:senderEmail, subject:subjectStr, content:contentStr}
        else
            set theMessage to make new outgoing message with properties {visible:false, subject:subjectStr, content:contentStr}
        end if
