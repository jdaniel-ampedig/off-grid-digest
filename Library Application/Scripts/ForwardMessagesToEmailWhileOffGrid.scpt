...
-- Returns {enabled:boolean, startStr:text, endStr:text}
-- Returns {enabled:boolean, startStr:text, endStr:text}
on readConfig()
    try
        ...
        my logRun("readConfig: enabled=" & (enabledBool as text) & " startStr=" & startStr & " endStr=" & endStr)
        return {enabledBool, startStr, endStr}
...
"
		...
		set subjectStr to "Unread 1:1 Messages (" & totalCount & ") in " & (count of convNames) & " Conversation" & (my pluralSuffix(count of convNames))
