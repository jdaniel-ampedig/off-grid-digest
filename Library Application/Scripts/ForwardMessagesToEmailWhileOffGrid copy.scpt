...
on fetchMissedCallsSince(lastCID)
    try
        -- Some macOS versions use CoreData tables: ZCALLRECORD etc.
        -- We'll attempt a best-effort query. If it fails, we log and return {}.
        set sql1 to "
WITH rec AS (
  SELECT
    zr.Z_PK AS cid,
...
WITH msgs AS (
  SELECT
    ...
    -- Prefer plain text; normalize newlines. (cached_text removed for compatibility)
...
"
		...
		if raw is "" then
			my logRun("Checked - no unread 1:1 inbound messages (within window)")
			return
		end if
		...
		repeat with r in rows
			if r is not "" then
				set AppleScript's text item delimiters to " || "
				set cols to text items of r
				-- Expect: rid, ts, chatname, sender, body, attachments, ablob
        ...
        if (do shell script "/usr/bin/test -r " & quoted form of callDBPath & " ; echo $?") is "0" then
            ...
            if (count of missedList) > 0 then
                ...
                -- Only advance the pointer if we’re going to send the email
        ...
        -- If for some reason no email set then do not try to send
